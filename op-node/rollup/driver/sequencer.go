...
func (d *Sequencer) PlanNextSequencerAction() time.Duration {
	...
	if safe {
		...
		return time.Duration(d.rollupCfg.MilliBlockInterval(buildingOnto.Timestamp())) * time.Millisecond
	}
	...
	blockTime := time.Duration(d.rollupCfg.MilliBlockInterval(head.Timestamp())) * time.Millisecond
	// payloadTime := time.Unix(int64(head.Time+d.rollupCfg.BlockTime), 0)
	payloadTime := time.UnixMilli(int64(d.rollupCfg.NextBlockMilliTimestamp(head)))
...
func (d *Sequencer) RunNextSequencerAction(ctx context.Context, agossip async.AsyncGossiper, sequencerConductor conductor.SequencerConductor) (*eth.ExecutionPayloadEnvelope, error) {
	...
	if onto, buildingID, safe := d.engine.BuildingPayload(); buildingID != (eth.PayloadID{}) || agossip.Get() != nil {
		if safe {
			...
			// approximates the worst-case time it takes to build a block, to reattempt sequencing after.
			d.nextAction = d.timeNow().Add(time.Millisecond * time.Duration(d.rollupCfg.MilliBlockInterval(onto.Timestamp())))
			return nil, nil
		}
		...
		if err != nil {
			if errors.Is(err, derive.ErrCritical) {
				...
				d.nextAction = d.timeNow().Add(time.Millisecond * time.Duration(d.rollupCfg.MilliBlockInterval(onto.Timestamp()))) // hold off from sequencing for a full block
