...
contract SystemConfig is OwnableUpgradeable, ISemver, IGasToken {
        ...
        initializer
    {
        ...
        _setStartBlock();
        console.log("_setGasPayingToken at: %s", _addresses.gasPayingToken);
    ...
    function _setGasPayingToken(address _token) internal virtual{
        if (_token != address(0) && _token != Constants.ETHER && !isCustomGasToken()) {
            console.log("_token, %s", _token);
            //console.log("_token, %s", ERC20(_token));
            console.log("_token, %s", ERC20(_token).name());
